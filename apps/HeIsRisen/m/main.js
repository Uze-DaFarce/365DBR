    ...
    if (!forceNew) {
        try {
            ...
            if (savedStateStr) {
                ...
                let isValidEggData = Array.isArray(savedState.eggData) && savedState.eggData.every(item => item && typeof item === 'object' && typeof item.eggId === 'number');
                let isValidSections = Array.isArray(savedState.sections) && savedState.sections.every(item => item && typeof item === 'object' && typeof item.name === 'string');
                let isValidFoundEggs = Array.isArray(savedState.foundEggs) && savedState.foundEggs.every(item => typeof item === 'number' || (item && typeof item === 'object' && typeof item.eggId === 'number'));
                let isValidStamped = Array.isArray(savedState.stampedSections) && savedState.stampedSections.every(item => typeof item === 'string');
                ...
                if (savedState && typeof savedState === 'object' && isValidEggData && isValidSections) {
                    ...
                    registry.set('stampedSections', isValidStamped ? savedState.stampedSections : []);
...
class MusicScene extends Phaser.Scene {
  constructor() {
    ...
    const getSafeVol = (key) => {
      ...
      let parsed = (val !== null && val !== undefined && String(val).trim() !== '' && typeof val !== 'object' && !Array.isArray(val)) ? Number(val) : NaN;
      if (isNaN(parsed) || parsed < 0 || parsed > 1) {
          let backupVal = null;
          try { backupVal = localStorage.getItem(key + '_backup'); } catch (e) { console.warn('localStorage error', e); }
          parsed = (backupVal !== null && backupVal !== undefined && String(backupVal).trim() !== '' && typeof backupVal !== 'object' && !Array.isArray(backupVal)) ? Number(backupVal) : NaN;
          if (isNaN(parsed) || parsed < 0 || parsed > 1) {
              return 0.5;
          }
      }
...
window.addEventListener('load', () => {
  ...
  const handleTouch = (e) => {
    ...
    if (isPortrait && isSmallScreen && !isLocked) {
      ...
      // ⚡ Bolt Optimization: Replace Array.from and .map with standard for loop to prevent closure allocations
      // in high-frequency touchmove events which cause garbage collection stutter.
      const changedTouchesLen = e.changedTouches.length;
      const touches = new Array(changedTouchesLen);
      const physicalWidth = window.innerWidth; // Hoist out of loop
