...
class Confirmation extends Phaser.GameObjects.Container {
  constructor(scene, x, y, text, onYes, onNo) {
    ...
    const yesBtnContainer = this.scene.add.container(
    ...
    const noBtnContainer = this.scene.add.container(
      this.scene.cameras.main.width / 2 + 100,
      this.scene.cameras.main.height / 2 + 50,
    );
...
function initializeGameData(registry, cache, forceNew = false) {
  if (!forceNew) {
    try {
      ...
      try {
        savedStateStr = localStorage.getItem("heIsRisenGameState");
      } catch (e) {
      ...
      if (savedStateStr) {
        ...
        if (
          ...
          Array.isArray(savedState.eggData) &&
          ...
          registry.set(
            "foundEggs",
            Array.isArray(savedState.foundEggs) ? savedState.foundEggs : [],
          );
          registry.set(
            "stampedSections",
            Array.isArray(savedState.stampedSections)
              ? savedState.stampedSections
              : [],
          );

          let loadedCorrect =
            savedState.correctCategorizations !== null &&
            savedState.correctCategorizations !== ""
              ? Number(savedState.correctCategorizations)
              : NaN;
          if (
            isNaN(loadedCorrect) ||
            !isFinite(loadedCorrect) ||
            loadedCorrect < 0
          )
            loadedCorrect = 0;
          ...
          let loadedScore =
            savedState.currentScore !== null && savedState.currentScore !== ""
              ? Number(savedState.currentScore)
              : NaN;
          ...
          try {
            ...
            let loadedScore =
              highScoreVal !== null && highScoreVal !== ""
                ? Number(highScoreVal)
                : NaN;
            if (
              isNaN(loadedScore) ||
              !isFinite(loadedScore) ||
              loadedScore < 0
            ) {
              loadedScore = 0;
            }
  ...
  if (
    ...
    if (validSymbols.length !== symbolsData.symbols.length) {
      console.warn(
        `Security: Filtered ${symbolsData.symbols.length - validSymbols.length} invalid symbols.`,
      );
      symbolsData.symbols = validSymbols;
    }
    registry.set("symbols", symbolsData);
  ...
  if (!Array.isArray(mapSections)) {
    console.warn(
      "Security: map_sections.json failed to load or is invalid. Using empty fallback.",
    );
    mapSections = [];
  }
  if (mapSections && mapSections.length > 0) {
    ...
    const eggs = Phaser.Utils.Array.Shuffle(
      Array.from({ length: TOTAL_EGGS }, (_, i) => i + 1),
    );
    const sections = mapSections.map((section) => ({
      name: section.name,
      eggs: [],
    }));

    let eggIndex = 0;
    const shuffledSymbols = Phaser.Utils.Array.Shuffle([
  ...
  try {
    ...
    let loadedScore =
      highScoreVal !== null && highScoreVal !== "" ? Number(highScoreVal) : NaN;
    if (isNaN(loadedScore) || !isFinite(loadedScore) || loadedScore < 0) {
      loadedScore = 0;
    }
    registry.set("highScore", loadedScore);
...
class CursorScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.fingerCursor = this.add
      .image(0, 0, "finger-cursor")
      .setOrigin(0, 0)
...
class MusicScene extends Phaser.Scene {
  constructor() {
    ...
    const getSafeVol = (key) => {
      ...
      if (isNaN(parsed) || parsed < 0 || parsed > 1) {
        let backupVal = null;
        try {
          backupVal = localStorage.getItem(key + "_backup");
        } catch (e) {
          console.warn("localStorage error", e);
        }
  ...
  create() {
    ...
    if (!music) {
      this.sound
        .add("background-music", { loop: true, volume: this.musicVolume })
        .play();
    ...
    if (this.registry.has("ambientVolume"))
      this.ambientVolume = this.registry.get("ambientVolume");
  ...
  scheduleAmbientSound() {
    ...
    this.time.delayedCall(delay, () => {
      ...
      const ambientSound = this.sound.add(randomAmbient, {
...
class UIScene extends Phaser.Scene {
  ...
  resize(gameSize) {
    ...
    if (this.settingsContainer) {
      ...
      let controlXOffsets = isTwoColumn
        ? [panelCenterX - 150, panelCenterX - 150, panelCenterX + 150]
        : [panelCenterX, panelCenterX, panelCenterX];
      ...
      this.settingsContainer.getAll().forEach((child) => {
        ...
        // Re-center "START NEW GAME" button container
          ...
          child.type === "Container" &&
          child.list.length > 1 &&
          child.list[1].type === "Text" &&
          child.list[1].text === "START NEW GAME"
        ) {
        ...
        // Controls logic: Label, Left Arrow, Right Arrow, Value Text
          ...
          child.type === "Text" &&
          ["Music", "Ambient", "SFX"].includes(child.text)
        ) {
          child.setPosition(
            controlXOffsets[controlLabelIndex],
            targetY + controlYOffsets[controlLabelIndex] - 30,
          );
          controlLabelIndex++;
        }
        // Number Value Text (matches 0-10 format, typically length 1 or 2 digits)
          ...
          child.setPosition(
            controlXOffsets[controlValueIndex],
            targetY + controlYOffsets[controlValueIndex],
          );
          controlValueIndex++;
        }
        // Left Arrow Container
          ...
          child.setPosition(
            controlXOffsets[controlArrowLeftIndex] - 50,
            targetY + controlYOffsets[controlArrowLeftIndex],
          );
        ...
        // Right Arrow Container
          ...
          child.setPosition(
            controlXOffsets[controlArrowRightIndex] + 50,
            targetY + controlYOffsets[controlArrowRightIndex],
          );
  ...
  createGearIcon() {
    ...
    gearContainer.on("pointerover", () =>
      this.tweens.add({
        targets: gearContainer,
        scaleX: gearContainer.baseScaleX * 1.2,
        scaleY: gearContainer.baseScaleY * 1.2,
        duration: 100,
        ease: "Sine.easeInOut",
      }),
    );

    gearContainer.on("pointerout", () =>
      this.tweens.add({
        targets: gearContainer,
        scaleX: gearContainer.baseScaleX,
        scaleY: gearContainer.baseScaleY,
        duration: 100,
        ease: "Sine.easeInOut",
      }),
    );
      ...
      this.tweens.add({
        ...
        onComplete: () => {
          this.time.delayedCall(50, () => {
            this.openSettings();
            gearContainer.setScale(
              gearContainer.baseScaleX,
              gearContainer.baseScaleY,
  ...
  createSettingsPanel() {
    ...
    this.settingsContainer = this.add
      .container(0, 0)
      .setVisible(false)
      .setDepth(100);
    ...
    const overlay = this.add
      .rectangle(0, 0, sw, sh, 0x000000, 0.7)
      .setOrigin(0)
    ...
    const panel = this.add
      .rectangle(sw / 2, sh / 2, width, height, 0x333333)
      .setStrokeStyle(4, 0xffffff);
    this.settingsContainer.add(panel);

    // Title
    const title = this.add
      .text(sw / 2, y + 40, "Audio Settings", {
        fontSize: "32px",
        fontFamily: "Comic Sans MS",
        fill: "#ffffff",
      })
    ...
    resetBtnContainer.on("pointerdown", () => {
      const confirmation = new Confirmation(
        ...
        () => {
          ...
          if (this.scene.isActive("SectionHunt"))
            this.scene.stop("SectionHunt");
  ...
  createNumberControl(label, centerX, y, type) {
    ...
    let currentVol = 0.5;
    if (this.registry.has(`${type}Volume`))
      currentVol = this.registry.get(`${type}Volume`);

    // Convert 0.0-1.0 to 0-10
    let displayValue = Math.round(currentVol * 10);

    const valueText = this.add
      .text(centerX, y, displayValue.toString(), {
        fontSize: "32px",
        fontFamily: "Comic Sans MS",
        fill: "#ffff00",
        fontStyle: "bold",
...
class MainMenu extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.atlas(
      "level-complete-atlas",
      "assets/video/level-complete.png",
      "assets/video/level-complete.json",
    );
    this.load.image(
      "level-complete-stamp",
      "assets/objects/level-complete-stamp.png",
    );
    this.load.image(
      "finger-cursor",
      "assets/cursor/pointer-finger-pointer.png",
    );
    ...
    this.load.atlas(
      "egg-cellent-button",
      "assets/objects/egg-cellent.png",
      "assets/objects/egg-cellent.json",
    );
    this.load.atlas(
      "eggs-tra-stinky-button",
      "assets/objects/eggs-tra-stinky.png",
      "assets/objects/eggs-tra-stinky.json",
    );
    this.load.image(
      "symbol-result-summary-diag",
      "assets/objects/symbol-result-summary-diag.png",
    );
    ...
    this.load.on("filecomplete-json-map_sections", (key, type, data) => {
      if (Array.isArray(data)) {
        data.forEach((section) => {
          ...
          this.load.image(
            `${section.name}-thumb`,
            `assets/map/sections/${section.background}`,
          );
      ...
      if (data && data.symbols) {
        data.symbols.forEach((symbol) => {
          ...
          if (this.isValidSymbol(symbol)) {
            ...
            console.warn(
              `Security: Skipped invalid symbol filename: ${symbol.filename}`,
            );
          }
        });
      }
  ...
  create() {
    ...
    try {
      hasSaveState = localStorage.getItem("heIsRisenGameState") !== null;
    } catch (e) {
      console.warn("localStorage error", e);
    }

    const startBtnContainer = this.add
      .container(btnX, btnY)
      .setVisible(false)
      .setDepth(101);
    ...
    if (!this.registry.has("musicVolume"))
      this.registry.set("musicVolume", getSafeVol("musicVolume"));
    if (!this.registry.has("ambientVolume"))
      this.registry.set("ambientVolume", getSafeVol("ambientVolume"));
    if (!this.registry.has("sfxVolume"))
      this.registry.set("sfxVolume", getSafeVol("sfxVolume"));
...
class MapScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (!this.textures.exists("sparkle")) {
      ...
      const starObject = new Phaser.GameObjects.Star(
        this,
        10,
        10,
        4,
    ...
    mapSections.forEach((section) => {
      ...
      shadow.fillRoundedRect(
        -section.coords.width / 2 + 4,
        -section.coords.height / 2 + 4,
        section.coords.width,
        section.coords.height,
        radius,
      );
      ...
      border.fillRoundedRect(
        -section.coords.width / 2 - 5,
        -section.coords.height / 2 - 5,
        section.coords.width + 10,
        section.coords.height + 10,
        radius + 2,
      );
      ...
      maskGraphics.fillRoundedRect(
        -section.coords.width / 2,
        -section.coords.height / 2,
        section.coords.width,
        section.coords.height,
        radius,
      );
      ...
      const hitArea = this.add.rectangle(
        0,
        0,
        section.coords.width + 40,
        section.coords.height + 40,
        0x000000,
        0,
      );
      ...
      const isCompleted =
        ...
        sectionEggs.every((e) =>
          foundEggs.some(
            (found) => found === e.eggId || (found && found.eggId === e.eggId),
          ),
        );
      ...
      if (isCompleted) {
        if (!stampedSections.includes(section.name)) {
          ...
          const stampAnim = this.add.sprite(
            thumb.x,
            thumb.y,
            "level-complete-atlas",
          );
          ...
          // Play audio
          const sfxVol =
            this.registry.get("sfxVolume") !== undefined
    ...
    this.eggsAmminHaul = this.add
      .image(0, 0, "eggs-ammin-haul")
      .setOrigin(0, 0)
      .setInteractive()
    ...
    this.scoreText = this.add.text(0, 0, `${foundEggs}/${TOTAL_EGGS}`, {
      fontSize: "42px",
      fill: "#000",
      fontStyle: "bold",
      fontFamily: "Comic Sans MS",
    ...
    if (this.mapZones) {
      ...
      for (let m_idx = 0; m_idx < this.mapZones.length; m_idx++) {
        ...
        if (thumb.maskGraphics) {
          thumb.maskGraphics.setPosition(
...
class SectionHunt extends Phaser.Scene {
  ...
  checkLevelComplete(immediate = false) {
    ...
    if (foundEggs.length === TOTAL_EGGS) {
      announceToScreenReader(
        "All 60 Eggs Found! Transporting to the EggZam Room...",
      );
    ...
    if (currentSection) {
      ...
      const remainingCount = currentSection.eggs.filter(
        (id) => !foundIds.includes(id),
      ).length;
      if (remainingCount === 0) {
        announceToScreenReader(
          "Great Job Detective!! You found all the hidden eggs on this map, the others are hidden in other maps.",
        );
        const clearText = this.add
          .text(
            this.scale.width / 2,
  ...
  showCollectionFeedback(x, y, eggTexture, symbolTexture) {
    ...
    this.tweens.add({
      ...
      y: y - 180,
      scaleX: {
        value: eggSprite.scaleX * 2.5,
        duration: 400,
        ease: "Back.easeOut",
      },
      scaleY: {
        value: eggSprite.scaleY * 2.5,
        duration: 400,
        ease: "Back.easeOut",
      },
    ...
    if (symbolTexture) {
      const symSprite = this.add
        .image(x, y, symbolTexture)
        .setDepth(21)
        .setDisplaySize(50, 75);
      this.tweens.add({
        targets: symSprite,
        y: y - 180,
        scaleX: {
          value: symSprite.scaleX * 2.5,
          duration: 400,
          ease: "Back.easeOut",
        },
        scaleY: {
          value: symSprite.scaleY * 2.5,
          duration: 400,
          ease: "Back.easeOut",
        },
        angle: { value: 360, duration: 600, ease: "Quad.easeOut" },
  ...
  showIdleHint() {
    ...
    const foundIds = foundEggs.map((e) => e.eggId);
    const remainingCount = eggsInSection.filter(
      (id) => !foundIds.includes(id),
    ).length;
    ...
    if (remainingCount > 0) {
      ...
      const hintText = this.add
        .text(
          ...
          `Hint: ${remainingCount} eggs left here!`,
          {
            fontSize: "32px",
            fontFamily: "Comic Sans MS",
            fill: "#ffffff",
  ...
  create() {
    ...
    if (!this.registry.get("eggzamVideosLoaded")) {
      ...
      this.load.video(
        "eggzam-eggcellent",
        "assets/video/eggzam-eggcellent.mp4",
      );
      this.load.video("eggzam-stinky", "assets/video/eggzam-stinky.mp4");
      this.load.video(
        "eggzam-incorrect",
        "assets/video/eggzam-incorrect-classification.mp4",
      );
      this.load.start();
    }
    ...
    if (this.textures.exists(thumbKey)) {
      this.sectionImage = this.add
        .image(width / 2, height / 2, thumbKey)
    ...
    if (useVideo) {
      ...
      this.sectionVideo = this.add
        .video(width / 2, height / 2, videoKey)
        .setDisplaySize(1280 * scale, 720 * scale)
        .setDepth(0);
      ...
      const applySectionVideoScale = () => {
        if (
          this.sectionVideo &&
          this.sectionVideo.active &&
          this.sectionVideo.width > 0
        ) {
      ...
      const ambientVol = this.registry.has("ambientVolume")
        ? this.registry.get("ambientVolume")
        : 0.5;
      ...
      this.sectionVideo.on("loop", () => {
        ...
        if (this.sectionVideo.loopCount % 5 !== 0) {
          ...
          const currentAmbientVol = this.registry.has("ambientVolume")
            ? this.registry.get("ambientVolume")
            : 0.5;
      ...
      this.sectionVideo.on("error", () => {
        console.warn(
          `SectionHunt: Video ${videoKey} playback error. Falling back to thumbnail.`,
        );
    ...
    const eggDataArray = this.registry.get("eggData") || [];
    const sectionEggsData = eggDataArray.filter(
      (e) => e.section === this.sectionName && !e.collected,
    );
    ...
    this.eggZitButton = this.add
      .image(0, 200 * uiScale, "egg-zit-button")
      .setOrigin(0, 0)
    ...
    this.eggsAmminHaul = this.add
      .image(0, 350 * uiScale, "eggs-ammin-haul")
      .setOrigin(0, 0)
    ...
    this.scoreText = this.add
      .text(50 * uiScale, 98 * uiScale, `${foundEggs}/${TOTAL_EGGS}`, {
    ...
    this.zoomedView = this.add
      .renderTexture(0, 0, lensDiameter, lensDiameter)
      .setDepth(6)
      .setScrollFactor(0);
    this.zoomedView.setOrigin(0.5, 0.5); // Center origin

    this.maskGraphics = this.add
      .graphics()
      .fillCircle(0, 0, lensDiameter / 2)
      .setScrollFactor(0);
    this.zoomedView.setMask(this.maskGraphics.createGeometryMask());

    this.magnifyingGlass = this.add
      .image(0, 0, "magnifying-glass")
      .setOrigin(0.25, 0.25)
      .setDepth(7)
      .setScrollFactor(0);
    ...
    const key = this.isUsingVideo
      ? "placeholder-bg"
      : this.sectionImage
        ? this.sectionImage.texture.key
        : this.sectionName;
  ...
  update() {
    ...
    this.magnifyingGlass.setPosition(
      pointer.x + glassOffsetX,
      pointer.y + glassOffsetY,
    );
    ...
    if (this.isUsingVideo && this.sectionVideo && this.sectionVideo.active) {
      ...
      this.renderStamp.setTexture(
        this.sectionVideo.texture.key,
        this.sectionVideo.frame.name,
      );
    } else {
      // Use static image texture
      const key = this.sectionImage
        ? this.sectionImage.texture.key
        : this.sectionName;
    ...
    const targetScaleX =
      ((1280 * this.bgScale) / this.renderStamp.width) * zoom;
    const targetScaleY =
      ((720 * this.bgScale) / this.renderStamp.height) * zoom;
    ...
    for (let i = children.length - 1; i >= 0; i--) {
      ...
      if (egg && egg.active) {
        ...
        if (alpha > 0) {
          ...
          if (egg.symbolSprite && egg.symbolSprite.active) {
            this.eggStamp.setTexture(
              egg.symbolSprite.texture.key,
              egg.symbolSprite.frame.name,
            );
            this.eggStamp.setScale(
              egg.symbolSprite.scaleX * zoom,
              egg.symbolSprite.scaleY * zoom,
            );
    ...
    if (this.isUsingVideo && this.sectionVideo && this.sectionVideo.active) {
      if (this.sectionVideo.width > 0 && this.sectionVideo.height > 0) {
        ...
        if (Math.abs(this.sectionVideo.displayWidth - targetDisplayW) > 5) {
          this.sectionVideo.setDisplaySize(
            1280 * targetScale,
            720 * targetScale,
          );
          this.sectionVideo.setPosition(width / 2, height / 2);
...
class EggZamRoom extends Phaser.Scene {
  playGoodEggAnimation(eggImage, symbolImage, onCompleteCallback) {
    ...
    const scale = Math.min(
      this.sys.game.canvas.width / 1280,
      this.sys.game.canvas.height / 720,
    );
  ...
  playBadEggAnimation(eggImage, symbolImage, onCompleteCallback) {
    ...
    const musicScene = this.scene.get("MusicScene");
    if (musicScene) {
      const fartSound = this.sound.add("fart", {
        volume: this.registry.get("sfxVolume") ?? 0.5,
      });
      fartSound.play();
    }

    const gasParticles = this.add
      .particles(0, 0, "green-gas", {
        x: startX,
        y: startY,
  ...
  resetAmbientTimer() {
    ...
    const playAmbient = () => {
      ...
      this.playVideo(
        this.registry.get("lastAmbient") === 1
          ? "eggzam-ambient-2"
          : "eggzam-ambient-1",
        () => {
          this.registry.set(
            "lastAmbient",
            this.registry.get("lastAmbient") === 1 ? 2 : 1,
          );
          this.resetAmbientTimer();
        },
      );
  ...
  playVideo(videoKey, onComplete) {
    ...
    if (!this.cache.video.exists(videoKey)) {
      console.warn(
        `Video ${videoKey} not found in cache. Skipping video and firing complete.`,
      );
  ...
  create() {
    ...
    this.add
      .image(width / 2, height / 2, "eggzam-keyframe")
    ...
    this.scoreText = this.add
      .text(40 * uiScale, 90 * uiScale, `${foundEggsCount}/${TOTAL_EGGS}`, {
        fontSize: `${42 * uiScale}px`,
        fill: "#000",
        fontStyle: "bold",
        fontFamily: "Comic Sans MS",
    ...
    this.correctText = this.add
      .text(
        100 * uiScale,
        150 * uiScale,
        `Correct: ${this.registry.get("correctCategorizations")}`,
        {
    ...
    const showExplanation = (isCorrect, guessText) => {
      ...
      const executeExplanationPopup = () => {
        ...
        this.explanationText = this.add
          .container(offsetX + 640 * uiScale, offsetY + 360 * uiScale)
          .setDepth(100);
        ...
        const title = this.add
          .text(0, -bgHeight * 0.42, data.name || "Symbol", {
        ...
        const guessDisplay = this.add
          .text(0, -bgHeight * 0.33, `Your Guess: ${guessText}`, {
        ...
        const resultText = this.add
          .text(0, -bgHeight * 0.25, isCorrect ? "Correct!" : "Incorrect!", {
        ...
        const expText = this.add
          .text(0, -bgHeight * 0.04, data.explanation, {
            fontSize: `${28 * uiScale}px`,
            fill: "#000",
            fontFamily: "Comic Sans MS",
            wordWrap: { width: bgWidth * 0.9, useAdvancedWrap: true },
            align: "center",
          })
          .setOrigin(0.5);
        ...
        scriptures.forEach((scripture, index) => {
          const verseText = this.add
            .text(currentX, scriptureY, scripture, {
              fontSize: `${24 * uiScale}px`,
              fill: "#0000ee",
              fontStyle: "italic",
              fontFamily: "Comic Sans MS",
          ...
          verseText.on("pointerdown", (p, x, y, event) => {
            ...
            if (link) {
              ...
              iframeOverlay.style.setProperty(
                "cursor",
                "url('assets/cursor/pointer-finger-pointer.png'), auto",
                "important",
              );
          ...
          if (index < scriptures.length - 1) {
            const commaText = this.add
              .text(currentX, scriptureY, ", ", {
                fontSize: `${24 * uiScale}px`,
                fill: "#000",
                fontStyle: "italic",
                fontFamily: "Comic Sans MS",
        ...
        const eggImg = this.add
          .image(eggX, cornerY + eggSizeH / 2, `egg-${eggId}`)
          .setDisplaySize(eggSizeW, eggSizeH);

        // Symbol image if exists
        let symbolImgSmall = null;
        if (data && data.filename && this.textures.exists(data.filename)) {
          symbolImgSmall = this.add
            .image(eggX, cornerY + eggSizeH / 2, data.filename)
            .setDisplaySize(eggSizeW, eggSizeH);
        }
        ...
        const closeBtnContainer = this.add.container(
          closeBtnX,
          cornerY + closeBtnSize / 2,
        );
        ...
        closeBtnContainer.on("pointerdown", () => {
          if (
            ...
            this.currentVideo.video.src.includes("ambient")
          ) {
      ...
      if (isCorrect) {
        if (data.category === "Christian") {
          this.playGoodEggAnimation(
            this.displayedEggImage,
            this.displayedSymbolImage,
            () => executeExplanationPopup(),
          );
        } else if (data.category === "Pagan") {
          this.playBadEggAnimation(
            this.displayedEggImage,
            this.displayedSymbolImage,
            () => executeExplanationPopup(),
          );
    ...
    const stinkyBtn = this.add
      .sprite(
        centerBottomX - buttonSpacing,
        centerBottomY,
        "eggs-tra-stinky-button",
        "Symbol 10000",
      )
    ...
    stinkyBtn.on("pointerdown", () => {
      ...
      if (
        this.currentEgg &&
        !this.currentEgg.categorized &&
        !this.explanationText?.active &&
        !this.currentVideo
      ) {
        showExplanation(
          this.currentEgg.symbolData.category === "Pagan",
          "Eggs-tra Stinky",
        );
      }
    });
    ...
    eggCellentBtn.on("pointerdown", () => {
      ...
      if (
        ...
        showExplanation(
          this.currentEgg.symbolData.category === "Christian",
          "Egg-cellent",
        );
      }
    });
  ...
  displayRandomEggInfo(offsetX, offsetY, scale) {
    ...
    if (this.currentEgg === null || this.currentEgg.categorized) {
      ...
      if (uncategorizedEggs.length > 0) {
        ...
        this.noEggsText = this.add
          .text(offsetX + 420 * scale, offsetY + 220 * scale, ctaText, {
        ...
        if (foundEggs.length === TOTAL_EGGS) {
          // Summary Panel
          const summaryContainer = this.add
            .container(offsetX + 420 * scale, offsetY + 300 * scale)
            .setDepth(90);

          const holyEggs = foundEggs.filter(
            (e) => e.symbolData && e.symbolData.category === "Christian",
          ).length;
          const worldlyEggs = foundEggs.filter(
            (e) => e.symbolData && e.symbolData.category === "Pagan",
          ).length;
          ...
          const titleText = this.add
            .text(20 * scale, 30 * scale, "Final EggZam!", {
          ...
          const holyText = this.add
            .text(
              panelWidth / 2,
              75 * scale,
              `Egg-cellent Eggs: ${holyEggs} / 30`,
              {
          ...
          const worldlyText = this.add
            .text(
              panelWidth / 2,
              115 * scale,
              `Eggs-tra Stinky Eggs: ${worldlyEggs} / 30`,
              {
                fontSize: `${24 * scale}px`,
                fill: "#d32f2f",
                fontStyle: "bold",
          ...
          const totalText = this.add
            .text(panelWidth / 2, 155 * scale, `Total Categorized: 60/60`, {
              fontSize: `${24 * scale}px`,
              fill: "#000000",
              fontStyle: "bold",
              fontFamily: "Comic Sans MS",
          ...
          const playBtnContainer = this.add
            .container(panelWidth / 2 - 125 * scale, 180 * scale)
            .setDepth(101);
          ...
          const playBtnText = this.add
            .text(playBtnWidth / 2, playBtnHeight / 2, "PLAY AGAIN", {
              fontSize: `${24 * scale}px`,
              fill: "#000",
              fontStyle: "bold",
              fontFamily: "Comic Sans MS",
    ...
    if (this.currentEgg) {
      ...
      if (this.textures.exists(`egg-${eggId}`)) {
        this.displayedEggImage = this.add
          .image(eggPosX, eggPosY, `egg-${eggId}`)
      ...
      if (
        symbolData &&
        symbolData.filename &&
        this.textures.exists(symbolData.filename)
      ) {
        this.displayedSymbolImage = this.add
          .image(symbolPosX, symbolPosY, symbolData.filename)
          .setDisplaySize(eggScaleTarget, eggHeightTarget)
  ...
  button.on("pointerdown", () => {
    ...
    if (musicScene && musicScene.scene.isActive()) {
      ...
      scene.sound.play(soundKey, {
        volume: scene.registry.get("sfxVolume") ?? 0.5,
      });
    }
...
function addTooltip(scene, object, text) {
  ...
  object.on("pointerover", (pointer) => {
    ...
    tooltipContainer = scene.add.container(pointer.x, pointer.y - 30, [
      bg,
      textObj,
    ]);
