...
// 2. showExplanation
const showStart = content.indexOf("const showExplanation = (isCorrect, guessText) => {");
if (showStart !== -1) {
    // Find the end of `this.sound.play('error'); }`
    const errIdx = content.indexOf("this.sound.play('error');", showStart);
    const showEnd = content.indexOf("}", errIdx) + 1; // get the closing brace of the `else` block
    ...
    content = content.substring(0, showStart) + newShow + content.substring(showEnd);
}

// 3. End logic
const eggZamIdx = content.indexOf("class EggZamRoom");
// Find the exact block to replace
const endStrStart = "this.currentEgg = null;";
const endStrEnd = "return;";

const endStartIdx = content.indexOf(endStrStart, eggZamIdx);
// The reload function closes with `}); }`
// We need to find the `return;` after the reload block.
const reloadIdx = content.indexOf("window.location.reload();", endStartIdx);
if (reloadIdx !== -1) {
    const returnIdx = content.indexOf("return;", reloadIdx);
    const endEndIdx = returnIdx + 7; // up to and including 'return;'
