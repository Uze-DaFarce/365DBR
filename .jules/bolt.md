...
## 2026-03-31 - Mobile Canvas Touch Event Optimization
**Learning:** High-frequency event listeners on mobile canvas setups (like `touchmove` handled via synthetic events for portrait lock rotation) suffer massive GC overhead when heavily utilizing closure-based array mapping like `Array.from(touches).map(touch => new Touch(...))`. The mobile device has to allocate huge numbers of array iterators and anonymous functions per frame.
**Action:** Always replace `.map()` and `.forEach()` in high-frequency global event listeners with standard `for` loops pre-allocated using `new Array(length)`. Hoist expensive DOM layout reads like `window.innerWidth` outside of the loop.
