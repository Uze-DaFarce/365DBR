...
## 2026-04-02 - Array.from().map() in High-Frequency Touch Listeners
**Learning:** In mobile UI patterns, reading touch events inside a `touchmove` listener runs 60-120 times a second. Using `Array.from(e.changedTouches).map(...)` inside this listener causes two major performance issues: it creates constant garbage collection pressure due to array allocation and closure instantiation, and it often accompanies repeated DOM layout reads (e.g. `window.innerWidth`), which can trigger layout thrashing if any visual DOM changes happen during the touch loop.
**Action:** Replace `Array.from().map()` with a pre-allocated array (`new Array(e.changedTouches.length)`) and a standard `for` loop in all high-frequency touch or update handlers. Furthermore, explicitly hoist DOM properties like `window.innerWidth` outside of the loop to read them exactly once per event.
