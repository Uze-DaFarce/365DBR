...
## 2026-04-18 - Prevent Focal Verse Cutoff on Mobile Landscape
**Learning:** When programmatically scrolling to elements in constrained viewports (like mobile landscape), calculating a generic top-offset (e.g., `-125px`) often causes multi-line translation blocks to run below the footer. Relying solely on `alignBottomTarget` can also dangerously push the top of massive verses underneath sticky headers.
**Action:** Use a dynamic bounding box check. Calculate the `minTargetY` (highest allowed scroll that keeps the top visible under the header) and the `alignBottomTarget` (scroll needed to clear the footer). Set the final scroll position to `Math.min(targetY, maxAllowedTargetY)` to ensure the bottom clears the footer without sacrificing the top visibility of the verse. Never remove native `scroll-mt` utility classes, as they handle non-programmatic anchor layouts.
