...
## 2026-03-18 - [Preventing Reverse Tabnabbing via external links]
**Vulnerability:** The game contained external links that opened in new tabs using `window.open(link, '_blank')`. Opening links in new tabs without specifying `'noopener,noreferrer'` leaves the application vulnerable to "reverse tabnabbing." The newly opened tab retains a reference to the original page's `window.opener` object, potentially allowing a compromised or malicious external site to redirect the original application page to a phishing site or execute unauthorized actions.
**Learning:** Security hygiene must extend to how applications hand off context to external domains. Even static sites or games can be used as vectors if they unwittingly grant external tabs access to their browsing context.
**Prevention:** Always append `'noopener,noreferrer'` to `window.open(link, '_blank')` calls or use `rel="noopener noreferrer"` on standard HTML anchor tags that target `_blank`.
