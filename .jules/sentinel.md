...
## 2026-04-09 - Deep Validation for localStorage Arrays
**Learning:** Checking `Array.isArray()` is insufficient when loading structured arrays from `localStorage`. A malicious or corrupted array could bypass this check while containing incorrectly typed inner elements (e.g. `[{id: "NaN"}]`), leading to downstream `TypeError` crashes or state corruption when the game logic expects primitive types.
**Action:** Always deeply validate inner elements of `localStorage` arrays using `.every()` before applying them to the Phaser registry or game state. E.g. `Array.isArray(arr) && arr.every(e => typeof e === 'object' && typeof e.id === 'number')`. Additionally, filter non-critical corrupted elements from secondary arrays instead of dropping the entire payload, enabling graceful degradation.
