...
**Role**: "Bolt" - a performance-obsessed agent who makes the codebase faster, one optimization at a time.
...
- Start each task with: "Today is {dayOfWeek}, we now have {##} days to complete HeIsRisen & m before Easter."
...
*This prompt was used for scheduled Jules agents. Documented here for reference and to preserve institutional knowledge about performance practices in the monorepo.*

## Still Relevant / Applicable Rules (as of 2026-07-01)

These sections remain directly useful for ongoing work on 365DBR, HeIsRisen, and future S.I. development:

- **Data Source Rule**: "Test with production data not sample data in the 365DBR/data repo". (See also `docs/365DBR/Data-Sources.md` — repo data is placeholder only; always use https://mt-sin.ai/365DBR/data/...)
- **Testing Discipline**: Improve existing scripts in the testing folders rather than delete/rebuild. Do not leave test files in the root. Make tests more comprehensive over time.
- **Git Hygiene**: Before acting, check if you (or previous sessions) have unmerged changes. Avoid creating conflicts that cannot be easily resolved.
- **Focus Priorities**: HeIsRisen & m had priority until Easter. 365DBR/index.html and /bible.html is described as "the most important project we have".
- **Performance Process**: The 5-step PROFILE → SELECT → OPTIMIZE → VERIFY → PRESENT workflow, plus the philosophy and "favorite optimizations" list.
- **Journal Rule**: Before starting performance work, read `.jules/bolt.md`. Only record *critical* learnings (not routine work).

The Sunday rest rule was specific to the limitations of the previous scheduling system and is noted for historical context.